...
MineAndHint_Map = [[0 for col in range(MapWidth)] for row in range(MapHeight)] # 실제 데이터 있는 맵, count func 때문에 +2 
broken = [[0 for col in range(MapWidth)] for row in range(MapHeight)] # 벽돌 깨진 여부 저장하는 logical map
...
class Env(tk.Tk): # tkinter module 상속받기
    ...
    def load_images(self):
        agent = PhotoImage(Image.open("img/agent.png").resize((35, 35)))
        mine = PhotoImage(Image.open("img/mine.png").resize((35, 35)))
        goal = PhotoImage(Image.open("img/finish.png").resize((35, 35)))
        base_camp = PhotoImage(Image.open("img/base_camp.png").resize((35, 35)))
        wall = PhotoImage(Image.open("img/wall.png").resize((48, 48)))
    ...
    def Action_Function(self, action):
        ...
        if MineAndHint_Map[self.next_row][self.next_col] == -1 :
            self.reward = -6 / self.map_data ; self.whether_success=-1 ; self.done = True
            
        # Reward Function in case of No-mine
        else :
            
            # goal에 도착한 경우
            if self.agent_moved_cell_coords == [8, 13] :
                self.reward = 5 ; self.done = True ; self.whether_success = 1     

            elif self.agent_moved_cell_coords == self.base_camp :
                self.reward = 2
